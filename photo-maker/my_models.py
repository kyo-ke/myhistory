...
#models are called like model.forward(Variable(original_img))
"""
caffefunction is defined as 
class CaffeFunction(links.Chain):

you can check the model which you load by looking file which is placed github named model.prototxt.
then you can call layer by calling like model['layer']


prototxt is the file format for coding neural net which used in caffe
in prototxt layer is calculation blob is data
in layer bottom is input blob top is output blob

num_output : the number of filter
kernelsize : height and width of each filter
weight_filter : type and value of kernel
...
#        self.pool_func = F.max_pooling_2d
    ...
    def forward(self, x):
        ...
        y6 = self.model['conv6_4'](F.relu(F.dropout(self.model['conv6_3'](F.relu(self.model['conv6_2'](F.relu(self.model['conv6_1'](x5))))))))
