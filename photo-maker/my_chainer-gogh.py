...
# make teh styke image's aray (ch,ch)
#for making style mat , calculate correlation of the array 
def get_matrix(y):
    ch = y.data.shape[1]
    wd = y.data.shape[2]
    #make the array's shape (ch,wd,wd) to (ch,wd**2)
...
def generate_image(img_orig, img_style, width, nw, nh, max_iter, lr, img_gen=None):
    ...
    for i in range(max_iter):
        ...
        for l in range(len(y)):
            ...
            L1 = (l + 1)*0.3*np.float32(args.lam) * np.float32(nn.alpha[l])*F.mean_squared_error(y[l], Variable(mid_orig[l].data))
...
parser.add_argument('--initial_img','-ini', default = 'nothing', help = 'initial image')
...
c3 = np.zeros(3)
for i in range(3):
    c1[i] = np.mean(img_content[i])
    c2[i] = np.mean(img_style[i])
c1_args = c1.argsort()
c2_args = c2.argsort()
for i in range(3):
    c3[c1_args[i]] = c2_args[i]
img_content = img_content[c3,:,:]
